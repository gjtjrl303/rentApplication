...
public class CustomQueryView extends JFrame {
    ...
    private final SelectService selectService = new SelectService(new SelectDao());
    ...
    public CustomQueryView() {
        ...
        executeBtn.addActionListener(e -> {
            ...
            try {
                List<String[]> rows = selectService.executeSelect(query);
                List<String> columnList = selectService.getColumnNames(query);

                // 컬럼 벡터 생성
                Vector<String> columnNames = new Vector<>(columnList);
                ...
                JOptionPane.showMessageDialog(this, "쿼리 실행 중 오류: " + ex.getMessage(), "에러", JOptionPane.ERROR_MESSAGE);
                ex.printStackTrace();
            }
        });
